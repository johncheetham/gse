/*
    ...
    It came from gShogi and may have been modified for use in gse.
...
ShowResults(short score, unsigned short *bstline, char ch)
{
    ...
    if (et >= 100)
    {
        if (et != 0) nps = (NodeCnt * 100) / et;
        printf("info time %ld depth %d nodes %ld nps %ld score cp %d ",
               et * 10, Sdepth, NodeCnt, nps, score);
        /*printf("info depth %2d score %6d time %4ld nodes %8ld  ",
               Sdepth, score, et / 100, NodeCnt);*/
        ShowLine(bstline);
        fflush(stdout);
    }
